...
# wine_name = 'grigio'
# result = session.run(winestyle_find, simple_wine)
# print(result)
# winestyle  = asyncio.run(winestyle_find(wine_name))
# if winestyle:
#     global_result.extend(winestyle)
# simplewine = asyncio.run(simple_wine(wine_name))
# if simplewine:
#     global_result.extend(simplewine)
...
async def all_commands(message: types.Message):
    ...
    #await winestyle_find(message)
    await am_wine(message)

    # for i in result:
    #     await tprint(result, bot, message.chat_id)
